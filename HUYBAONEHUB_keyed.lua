--[[
  Simple key-gate inserted by ChatGPT.
  Required key: HBSCRIPT
  Paste this block at the very top of your script if using manually.
]]

local REQUIRED_KEY = "HBSCRIPT"

-- Allow users to auto-fill the key via getgenv()/shared if they want.
local function resolveProvidedKey()
    local ok, genv = pcall(getgenv)
    if ok and type(genv) == "table" and type(genv().HB_KEY) == "string" then
        return genv().HB_KEY
    end
    if type(shared) == "table" and type(shared.HB_KEY) == "string" then
        return shared.HB_KEY
    end
    return nil
end
...
local function makeGui()
    ...
    gui.Parent = (syn and syn.protect_gui and syn.protect_gui(gui)) or game:GetService("CoreGui")
    ...
    box.Position = UDim2.new(0, 16, 0, 54)
    ...
    msg.TextSize = 13
    msg.TextColor3 = Color3.fromRGB(255, 85, 85)
    msg.Position = UDim2.new(0, 16, 0, 140)
    msg.Size = UDim2.new(1, -32, 0, 20)
    msg.TextXAlignment = Enum.TextXAlignment.Left
    msg.Parent = frame

    return gui, frame, box, btn, msg
end

_G.__HB_AUTH__ = _G.__HB_AUTH__ or false

local provided = resolveProvidedKey()
if provided == REQUIRED_KEY then
    _G.__HB_AUTH__ = true
else
    local gui, frame, box, btn, msg = makeGui()

    local function try()
        if (box.Text or "") == REQUIRED_KEY then
            _G.__HB_AUTH__ = true
            ...
            msg.Text = "Key sai. Hãy thử lại."
            TweenService:Create(msg, TweenInfo.new(0.1), {TextTransparency = 0}):Play()
        end
    end
    btn.MouseButton1Click:Connect(try)
    box.FocusLost:Connect(function(enterPressed) if enterPressed then try() end end)
end

-- Block further execution until the correct key is provided.
while not _G.__HB_AUTH__ do task.wait() end
...
shared.LoaderTitle = "follow script rô bờ lốc";
...
v23.Draggable = true;
v23.Image = "http://www.roblox.com/asset/?id=3389799337";
v24.Parent = v23;
...
v16.Home:AddButton({
    Title = "Discord",
    Description = "discord to UDP",
    Callback = function()
        setclipboard("https://discord.gg/HUYBAONE");
    end
});
